...
public class Sistema {
    ...
    public boolean carregaDados(Usuario usuario){
        ...
        if(contaUsuario == null){
            contaUsuario = new Conta("Saldo", 0.0);
            criaConta(contaUsuario, usuario);
        }
    ...
    public ArrayList<Registro> filtraRegistros(String categoria, Conta conta){
    ...
    public boolean removeRegistro(Registro registro){
        //removo tanto do banco de dados quanto da cópia sendo usada agora
        usuarioLogado.getConta().getHistorico().remove(registro);
        return conexaoRegistro.remover(registro.getId());
    }
    ...
    public boolean removeOrcamentos(Orcamento orcamento){
        usuarioLogado.getConta().getOrcamentos().remove(orcamento);
        return conexaoOrcamento.remover(orcamento.getId());
    }
    // Categoria
    ...
    public ArrayList<Orcamento> filtraOrcamentos(String categoria, Conta conta){
    ...
    public boolean atualizaConta(Conta novaConta){
        novaConta.setId(usuarioLogado.getConta().getId());

        if(conexaoConta.alterar(novaConta)){
            usuarioLogado.getConta().setNome(novaConta.getNome());
            usuarioLogado.getConta().setSaldo(novaConta.getSaldo());
            return true;
        }
        return false;
    }
